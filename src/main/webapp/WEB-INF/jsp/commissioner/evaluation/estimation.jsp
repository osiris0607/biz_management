...
<script type='text/javascript'>
	...
	function searchEvaluationCommissionerDetailCB(data){
		...
		if ( data.result_data.chairman_yn == "Y") {
			...
			if (data.result_data.chairman_result == "부적합" ) {
				$("#rating_score_type_incongruity").prop("checked", true);
			} else {
				$("#rating_score_type_congruence").prop("checked", true);					
			}
			$("#chairman_comment").val(data.result_data.chairman_comment);
			
			if ( data.result_data.chairman_submit_yn == "Y") {
				str += "	<td><span>" + data.result_data.chairman_result + "</span></td>";
				str += "	<td>작성완료</td>";
				str += "	<td class='last'><span class='font_blue'>제출완료</span></td>";
			} else {
				str += "	<td><span></span></td>";
	...
	function signEvaluation() {
		var url = "/commissioner/fwd/evaluation/sign/evaluation?member_id=" + "${member_id}" + "&evaluation_id=" + "${vo.evaluation_id}";  
	...
	function chairmanRegistrationEvalutionResultItem(type) {
		...
		formData.append("item_type", evaluationItemInfo[0].item_type);
		formData.append("evaluation_id","${vo.evaluation_id}");
		formData.append("chairman_comment",$("#chairman_comment").val());
		formData.append("chairman_result",$("input[name='chairman_result']:checked").val());

		$.ajax({
		    type : "POST",
		    url : "/commissioner/api/evaluation/resultItem/chairmanRegistration",
		    data : formData,
		    processData: false,
		    contentType: false,
		    mimeType: 'multipart/form-data',
	...
	var finishSubmitChairmanEvaluation = false;
	function submitChairmanEvaluation(type) {
		// 모든 평가위원의 평가가 완료되어야 한다.
		if ( isAllEvaluationCompleted == false ) {
			showPopup("모든 평가위원의 평가서가 제출되지 않았습니다.");
			return;
		...
		formData.append("chairman_comment",$("#chairman_comment").val());
		formData.append("chairman_result",$("input[name='chairman_result']:checked").val());
		// 임시 저장이 아니면 위원장 평가서 Flag를 'Y'로 변경
		if ( type != 1) {
			formData.append("chairman_submit_yn", "Y");
		}
			
		$.ajax({
		    type : "POST",
		    url : "/commissioner/api/evaluation/update/submitChairmanEvaluation",
		    data : formData,
		    processData: false,
		    contentType: false,
		    mimeType: 'multipart/form-data',
		    success : function(data) {
		    	var jsonData = JSON.parse(data);
			    if ( jsonData.result == true) {
			    	showPopup("위원장 평가서 제출이 완료되었습니다.");
			    	finishSubmitChairmanEvaluation = true;
			    } else {
			    	showPopup("위원장 평가서 제출에 실패했습니다. 디시 시도해 주시기 바랍니다.");
			    }
		    },
		    error : function(err) {
		        alert(err.status);
	...
	function chairmanSignEvaluation() {
		...
		var url = "/commissioner/fwd/evaluation/sign/chairmanEvaluation?member_id=" + "${member_id}" + "&evaluation_id=" + "${vo.evaluation_id}";  
		window.open(url, "_blank", 'width=700, height=460'); 				
	}
	...
	function commonPopupConfirm(){
		...
		if ( finishSubmitChairmanEvaluation ) {
			$('.send_save_popup_box').fadeOut(350);
	    	location.reload();
		}
	    	...
	    	<div class="content_area">
				...
				<div class="content_area copmpany_area pt120" id="copmpany_area">							
					...
					<div class="table_area rating_result_tabel_area">
						<div class="table_area">
							...
							<div class="button_box clearfix fr pb20 list_btn mt30 rating_btn_hide">										
								...
								<button type="button" class="gray_btn fl" onclick="location.href='/commissioner/fwd/evaluation/main'">목록</button>
							</div>
					...
					<div class="table_area rating_leader_tabel_area">
						<div class="table_area">
								...
								<tfoot>											
									<tr>
										...
										<td class="last total_score" colspan="11">													
											<input type="radio" id="rating_score_type_congruence" value="적합" name="chairman_result" checked />
											<label for="rating_score_type_congruence" class="font_blue mr20">적합</label>
											<input type="radio" id="rating_score_type_incongruity" value="부적합" name="chairman_result" />
							...
							<table class="write fixed mt30">
								...
								<tbody>
									<tr>
										<th scope="row" class="ta_c"><label for="chairman_comment">의결 결과 및 종합의견</label></th>
										<td><textarea rows="10" class="w100" id="chairman_comment"></textarea></td>	
									</tr>											
								</tbody>
							</table>
							<!--//의결 사항-->
							...
							<div class="button_box clearfix fr pb20 list_btn leader_btn_hide">									
								<button type="button" class="blue_btn2 fl mr5" onclick="submitChairmanEvaluation(1);">임시저장</button>						
					...
					<div class="list_back clearfix mt50 list_back_btn">
						<button type="button" class="gray_btn fr" onclick="location.href='/commissioner/fwd/evaluation/main'">목록</button>
					</div>
				</div><!--//content_area-->
			<!--//평가 과제-->
			</div>
...
<div class="leader_send_save_popup_box">		
	...
	<div class="leader_send_save_popup">
		...
		<div class="popup_txt_area">
			<p tabindex="0"><span class="fw_b">제출 후에는 수정이 불가합니다.</span><br /><span class="font_blue">제출</span> 하시겠습니까?</p>
		    <div class="popup_button_area_center">
				<button type="submit" class="blue_btn popup_close_btn" onclick="submitChairmanEvaluation(2);">제출</button>
			</div>	
		</div>									
	</div> 
</div>
