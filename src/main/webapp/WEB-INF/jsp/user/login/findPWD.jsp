...
<html lang="ko">
	...
	<script type='text/javascript'>
		function createNewPwd() {
			var chkVal = ["member_id", "name", "mobile_phone_1", "mobile_phone_2", "mobile_phone_3"];
			...
			$.ajax({
			    ...
			    success : function(data) {
			    	...
			    	if (jsonData.result == true) {
			    		$("#find_password_area").show();
			    	}
			    	else {
			    		showPopup("입력하신 정보에 해당하는 계정이 존재하지 않습니다.", "아이디 찾기 안내");
			    	}
			    },
			    error : function(err) {
			        alert(err.status);
				...
				<section id="content">
					<div id="sub_contents">
						...
						<div class="content_area login_box_area">
							<h2 style="line-height: 140%;font-size: 24px;"><span class="font_blue">아이디, 이름, 휴대폰번호</span>를 통한 본인 확인 절차가 완료되면<br /> 등록하신 <span class="font_blue">이메일로 임시 비밀번호</span>가 발송 됩니다.</h2>
							<!--<p class="login_info">해당 아이디로 로그인 후 홈페이지를 이용하시기 바랍니다.</p>-->
							<ul class="login_class clearfix">								
								<li class="member_login_box" style="height: 499px">
									<div class="member_login">
										<img src="/assets/user/images/sub/member_login_img.png" alt="개인회원 로그인 이미지" />
										<h3 style="width:100%;text-align:center">비밀번호 찾기</h3>
										<ul class="login_form">
											<li>
												<form action="" class="w80 clearfix" style="margin: 0 auto;">
													<label for="member_id" class="fl mt10" style="margin-right: 47px;">아이디</label>
													<input type="text" title="아이디" id="member_id" class="fl" style="width: 73.7%;"  placeholder="아이디를 입력해 주세요." />
												</form>
											</li>
											<li class="mt10">
												<form action="" class="w80 clearfix" style="margin: 0 auto;">
													<label for="name" class="fl mt10" style="margin-right: 62px;">이름</label>
													<input type="text" title="이름" id="name" class="fl" style="width: 73.7%;" placeholder="이름을 입력해 주세요." />
												</form>												
											</li>
											<li class="mt10">
												<form action="" class="w80 clearfix" style="margin: 0 auto;">
													...
													<input type="tel" id="mobile_phone_2" maxlength="4" oninput="numberOnlyMaxLength(this);" class="form-control brc-on-focusd-inline-block w_6 fl d_input ls">
													<span style="display:block;" class="fl mc8">-</span>
													<label for="mobile_phone_3" class="hidden">전화</label>
													<input type="tel" id="mobile_phone_3" maxlength="4" oninput="numberOnlyMaxLength(this);" class="form-control brc-on-focusd-inline-block fl d_input ls" style="width:62px;">
												</form>
											</li>													
										</ul>
										<button type="button" class="btn btn_blue member_btn w80 mt30" onclick="createNewPwd();">비밀번호 찾기</button>	
									</div>
								</li>								
							</ul>
						</div>
						...
						<!--비밀번호 찾기 검색 완료-->
						<div class="content_area login_box_area" id="find_password_area" style="display:none">
							<h2 style="font-size: 24px;">등록된 <span class="font_blue">이메일로 임시 비밀번호</span>를 발급하였습니다. </h2>							
							<div class="btn_box clearfix mt20 mb20 w80" style="margin: auto;padding-top: 20px;">
								<button type="button" class="blue_btn fl w100" style="height: 50px" onclick="location.href='/user/fwd/login/memberLogin'" title="로그인화면 바로가기">로그인 페이지로</button>								
							</div>
						</div>
						<!--//비밀번호 찾기 검색 완료-->
