...
<script type='text/javascript'>
	...
	function searchList(pageNo) {
		...
		comAjax.setUrl("<c:url value='/admin/api/announcement/proposal/search/paging' />");
		comAjax.setCallback(searchListCB);
		comAjax.addParam("pageIndex", pageNo);
		comAjax.addParam("orderby", "DATE DESC");

		// 기술공모의 공고 페이지이므로 type은 D0000003 로 고정
		comAjax.addParam("type", "D0000004");
		var temp = $("#process_status option:selected").val();
	...
	function searchListCB(data) {
		...
		if (total == 0) {
			...
			$.each(data.result, function(key, value) {
				...
				str += "	<td><a href='/admin/fwd/announcement/proposal/detail?announcement_id=" + value.announcement_id + "'><div class='txt_hidden'>" + value.title + "</div></a></td>";
				...
				if ( value.process_status == "D0000001") {
					str += "	<td><a href='/admin/fwd/announcement/proposal/modification?announcement_id=" + value.announcement_id + "'><span class='font_blue'>"+ value.status_name +"</span></a></td>";
					str += "	<td><span>&nbsp;</span></td>";
				}
				// 작성완료
	...
	function updateProcessStatus(id, status) {
		...
		if (confirm('수정 하시겠습니까?')) {
			$.ajax({
			    type : "POST",
			    url : "/admin/api/announcement/proposal/update/processStatus",
			    data : formData,
			    processData: false,
			    contentType: false,
			    ...
			    <div class="list_search_table">
				    <div class="table_count_area">
					    <div class="count_area clearfix">
						    ...
						    <div class="fr">
						    <button type="button" class="fr blue_btn2 mr3" onclick="location.href='/admin/fwd/announcement/proposal/registration'">공고 등록</button>
						    </div>								   
					    </div>							   
				    </div>
