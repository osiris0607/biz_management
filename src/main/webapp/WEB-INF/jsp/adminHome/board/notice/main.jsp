...
<script type='text/javascript'>
	...
	function searchListCB(data) {
		...
		else 
		{
			...
			var date = formatDate(value.reg_date);
				...
				str += "	<td class='announcement_name write_title'><a href='/adminHome/board/notice/detail?board_id=" + value.board_id +"'>" + value.title + "</a></td>";
				...
				if ( value.open_yn == "Y")
				{
					str += "			<input type='radio' id='cbx_noticeOpened" + index + "' name='cbx_openYn" + index + "' checked='checked' onclick='btn_openYn_onclick(\"" + value.board_id + "\", \"Y\", \""+index+"\");'/>";
					str += "			<label for='cbx_noticeOpened" + index + "' class='mr10'>게시</label>";
					str += "			<input type='radio' id='cbx_noticeUnopened" + index + "' name='cbx_openYn" + index + "' onclick='btn_openYn_onclick(\"" + value.board_id + "\", \"N\", \""+index+"\");' />";
					str += "			<label for='cbx_noticeUnopened" + index + "'>미게시</label>";
				}
				else 
				{
					str += "			<input type='radio' id='cbx_noticeOpened" + index + "' name='cbx_openYn" + index + "' onclick='btn_openYn_onclick(\"" + value.board_id + "\", \"Y\", \""+index+"\");'/>";
					str += "			<label for='cbx_noticeOpened" + index + "' class='mr10'>게시</label>";
					str += "			<input type='radio' id='cbx_noticeUnopened" + index + "' name='cbx_openYn" + index + "' checked='checked' onclick='btn_openYn_onclick(\"" + value.board_id + "\", \"N\", \""+index+"\");'/>";
					str += "			<label for='cbx_noticeUnopened" + index + "'>미게시</label>";
				}
				str += "		</div>";
				str += "	</td>";
	...
	* 게시 / 미게시 Radio 버튼
	*******************************************************************************/
	function radioClick(id, value, index) {

		var radioCheck= $('.push_check:checked').length;
		
		if(radioCheck > 3){
			console.log('radioCheck : ', radioCheck);
			alert("게시할 항목은 3개까지입니다.");
			value = 'N';
			$("input[id='cbx_noticeUnopened" + index + "']:radio").prop('checked', true);

			
			console.log('id, value click --> ', id, value, index);
			return false;
			
		}else {
			//수정
			btn_openYn_onclick(id, value);
		}				
			...
			<div class="table_count_area">
				<div class="count_area clearfix mb10">
					...
					<div class="fr clearfix">
						...
						<a href="/adminHome/board/notice/registration" class="blue_btn link_btn fl">등록</a>											
					</div>
				</div>
			</div>
