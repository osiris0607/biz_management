...
<script type='text/javascript'>
	...
	function prepareRegistration(status) {
  		var chkVal = ["reg_no", "company_name", "company_address", "company_phone", "representative_name", "industry_type", "business_type", 
  					  ...
  					  "consulting_campus", "sevice_name", "sevice_description",  "tech_info_market_report",  "reception_rnd_text"];
		...
		if ( $("input:radio[name=con_reception_class_g]").is(":checked") == false ) {
			showPopup("기술컨설팅 구분 은(는) 필수입력입니다.", "접수 안내");
			return;
		}
		if ( gfn_isNull($("select[name=large_selector]").val()) ) {
			showPopup("국가과학기술분류 대분류 은(는) 필수입력입니다.", "접수 안내");
			return;
		}
		if ( gfn_isNull($("select[name=middle_selector]").val()) ) {
			showPopup("국가과학기술분류 중분류 은(는) 필수입력입니다.", "접수 안내");
			return;
		}
		if ( gfn_isNull($("select[name=small_selector]").val()) ) {
			showPopup("국가과학기술분류 소분류 은(는) 필수입력입니다.", "접수 안내");
			return;
		}
		if ( gfn_isNull($("select[name=4th_industry_selector]").val()) ) {
			showPopup("4차 산업혁명 기술분류 은(는) 필수입력입니다.", "접수 안내");
			return;
		}
		if ( $("input:radio[name=consulting_take_yn_radio]").is(":checked") == false ) {
			showPopup("사전 컨설팅 실시 여부는 필수입력입니다.", "접수 안내");
			return;
		}
		...
		if ( mChoiceExpertList == null || mChoiceExpertList.length == 0) {
	...
	function registration(status){
		...
		formData.append("tech_consulting_take_yn", $("input:radio[name=consulting_take_yn_radio]:checked").val() );
		//기술 정보
		...
		var index = 0;
		// 전문가 POOL에서 선택된 전문가
		$("#choiced_expert_list_body tr").each(function(){
			var td = $(this).children();
			var expert = new Object();
			expert.member_id = $("#choice_checkbox" + index).val();
			expert.national_science = td.eq(1).text();
			expert.research = td.eq(2).text();
			expert.name = td.eq(3).text();
			expert.institution_name = td.eq(4).text();
			expert.institution_department = td.eq(5).text();
			expert.mobile_phone = td.eq(6).text();
			expert.email = td.eq(7).text();

			expertList.push(expert);
	        index++;
		});
		// 직접 입력 전문가
		if ( isDirectChoiceExpert ) {
			var expert = new Object();
			expert.member_id = "";
			expert.national_science = "";
			expert.research = $("#direct_input_research").val();
			expert.name = $("#direct_input_name").val();
			expert.institution_name = $("#direct_company_name").val();
			expert.institution_department = $("#direct_companypart_name").val();
			expert.mobile_phone = $("#direct_companypart_phone").val() + "-" + $("#direct_companypart_phone2").val() + "-" + $("#direct_companypart_phone3").val();
			expert.email = $("#direct_companypart_email").val();
			expertList.push(expert);
		}
					...
					<div class="table_area">
					    ...
					    <table class="write fixed">
						   ...
						   <tbody>
							    <tr id="consulting_type_tr">
								   ...
								   <td>
										<input type="radio" id="research_reception_class" name="con_reception_class_g" value="창업아이디어" checked /> 
							   ...
							   <tr id="consulting_purpose_tr">
								   ...
								   <td>
								   		<input type="radio" id="research_company_reception_practice" name="consulting_take_yn_radio" value="실시" checked /> 
					    ...
					    <table class="write fixed expert_search_table">
						   ...
						   <tbody>
							   ...
							   <tr>
								   <th scope="row"><span class="icon_box"><label for="expert_research_sphere">연구 분야</label></span></th>
								   <td>
						...
						<div class="table_area clearfix">
							...
							<div class="clearfix fl mt10">
								<input type="checkbox" id="checkbox_txtwrite" class="expert_input_btn" />
								<label for="checkbox_txtwrite">직접입력 (검색 목록에 없을 경우 <span class="font_blue">직접 입력</span>해 주세요.)</label>
							</div>
							<button type="button" class="fr gray_btn fixed expert_input_table_txt_del mb10 mt10" onclick="initChoicedExpert();">초기화</button>
							<table class="list fixed expert_hope_search_table2 mt10 expert_input_table mb20">
								...
								<tbody>
									<tr>
										<td class="first">
											<div class="d_n">
												<input type="checkbox" id="checkbox3_1" />&nbsp;<label for="checkbox3_1">&nbsp;</label>
											</div>
										</td>
										<td>
											<label for="direct_input_research" class="hidden">연구분야</label>
											<textarea name="direct_input_research" id="direct_input_research" cols="30" rows="2" class="w100" ></textarea>						
										</td>													
										<td>
											<label for="direct_input_name" class="hidden">성명</label>
											<input type="text" id="direct_input_name" class="form-control w100" />		
										</td>
										<td>
											<label for="direct_company_name" class="hidden">기관명</label>
											<input type="text" id="direct_company_name" class="form-control w100" />
										</td>
										<td>
											<label for="direct_companypart_name" class="hidden">부서</label>
											<input type="text" id="direct_companypart_name" class="form-control w100" />
										</td>
										<td>
											<label for="direct_companypart_phone" class="hidden">휴대전화</label>
											<select name="direct_companypart_phone" id="direct_companypart_phone" class="w_8 fl d_input ls">
												<option value="010">010</option>
											</select>
											<span style="display:block;" class="fl mc8">-</span>
	
											<label for="direct_companypart_name2" class="hidden">전화</label>
											<input type="tel" id="direct_companypart_phone2" maxlength="4" class="form-control brc-on-focusd-inline-block w_6 fl" placeholder="">
											<span style="display:block;" class="fl mc8 ls">-</span>
	
											<label for="direct_companypart_name3" class="hidden">전화</label>
											<input type="tel" id="direct_companypart_phone3" maxlength="4" class="form-control brc-on-focusd-inline-block w_6 fl ls" placeholder="">
										</td>
										<td class="last">
											<label for="direct_companypart_email" class="hidden">이메일</label>
											<input type="text" id="direct_companypart_email" class="form-control w100" placeholder="ex)XXX@email.com" />
										</td>
									</tr>												
								</tbody>											
							</table>
	
							<div class="clearfix" style="text-align: center; display: none;" id="expert_select_btn_div">
								<button type="button" class="blue_btn select_write" onclick="directChoiceExpert();" >선택</button>
							</div>
						</div>
						<!--//희망전문가-->
						<div class="button_box clearfix fr pb20">
							...
							<button type="button" class="blue_btn2 fl mr5 openLayer5" onclick="prepareRegistration('D0000001');">임시저장</button>
							<button type="button" class="blue_btn fl mr5" onclick="prepareRegistration('D0000002');">작성완료</button>
...
<div class="reception_complete_popup_box">		
	...
	<div class="reception_complete_popup">
		<div class="popup_titlebox clearfix">
			<h4 class="fl">접수 제출하기 안내</h4>							
            <a href="javascript:void(0)" title="닫기" class="white_font close_btn logout_popup_close_btn popup_close_btn fr"><i class="fas fa-times"></i><span class="hidden">닫기</span></a>
		</div>					
		<div class="popup_txt_area">
			<span class="popup_titlebox_title">2021년 기술제안 사업 신청서 제출 </span>
			<div class="popup_titlebox_txt">
				<p><span class="font_blue">[예]</span> 버튼을 누르면 사업 신청서 제출이 완료되며, 수정은 불가능합니다.</p>							
				<p>제출 완료 후에는 접수 대기 상태이며, 담당자가 제출서류를 확인하여 이상이 없을 경우, <br />
					제출 완료될 예정이며, 접수번호가 부여됩니다.</p>
				<p class="font_blue fz_b"><span class="fw500 font_blue">사업 신청서를 제출하시겠습니까?</span></p>
...
<div class="matching_save_box" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 10000; display: none;">		
	...
	<div class="save_popup2_popup">
		...
		<div class="popup_txt_area">						
		    ...
		    <div class="popup_button_area_center">
				<button type="button" class="blue_btn mr5" onclick="registration('D0000003');">신청</button>
				<button type="button" class="gray_btn popup_close_btn">취소</button>
			</div>	
		</div>									
	</div> 
