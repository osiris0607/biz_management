...
<script type='text/javascript'>
	$(document).ready(function() {
		...
		$("#radio_company").click(function() {
			// 기관 정보 등록이 안되어 있으면 팝업 생성
			if ( gfn_isNull($("#reg_no").val()) == true ) {
				$(".institution_warning_box, .popup_bg").fadeIn(350);
				$("#radio_member").prop("checked", true);
				return;
			}
		...
		if ( status != 'D0000001') {
			...
			if ( $("#radio_company").is(":checked") ) {
				var chkVal = ["reg_no", "company_name", "company_address", "company_phone", "representative_name", "industry_type", "business_type", 
					  		  ...
					  		  "sevice_name", "sevice_description", "tech_info_feature", "tech_info_effect"];
			} else {
	...
	function registration(status){
		...
		formData.append("tech_info_effect", $("#tech_info_effect").val() );
		//제출 서류
		...
		formData.append("self_check_list_json", JSON.stringify(selfCheckList));
	 	...
	 	$.ajax({
		    type : "POST",
		    url : "/member/api/reception/contest/registration",
		    data : formData,
		    processData: false,
		    contentType: false,
						...
						<table class="write fixed">
							...
							<tbody>
								<tr>
									...
									<td>
										<input type="radio" id="radio_member" name="selector_class" value="개인" checked>
										<label for="radio_member">개인</label>		
										<input type="radio" id="radio_company" name="selector_class" value="기관">
										<label for="radio_company" class="mr20">기관</label>										
									</td> 
							    </tr>																					
							 </tbody>						 
						</table>
				...
				<div class="table_area">
					<div class="content_area member_area" id="member_area">	
						...
						<table class="write fixed">
							...
							<tbody>
								<tr id="service_name_tr">
								    <th scope="row">
								    	<span class="icon_box"><span class="necessary_icon">*</span>
								    	<label for="sevice_name">기술명</label></span>
							    	</th>
								    <td>
								    	<input type="text" id="sevice_name" title="기술명" class="form-control w100">
							    	</td> 
							    </tr>
								<tr id="sevice_description_tr">
								    <th scope="row">
								    	<span class="icon_box"><span class="necessary_icon">*</span>
								    	<label for="sevice_description">기술 개요</label></span>
							    	</th>
								    <td>
								    	<input type="text" id="sevice_description" title="기술 개요" class="form-control w100">
							    	</td> 
							    </tr>
						...
						<div class="button_box clearfix fr pb20">
							...
							<button type="button" class="blue_btn fl mr5" onclick="prepareRegistration('D0000002');">접수완료</button>
...
<div class="institution_warning_box" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 10000; display: none;">
	<div class="popup_bg"></div>
	<div class="save_popup2_popup">
		<div class="popup_titlebox clearfix">
			<h4 class="fl">기관정보 안내</h4>							
            <a href="javascript:void(0)" title="닫기" class="white_font close_btn logout_popup_close_btn popup_close_btn fr"><i class="fas fa-times"></i><span class="hidden">닫기</span></a>
		</div>					
		<div class="popup_txt_area">						
			<p tabindex="0">기관정보가 등록되어 있지 않습니다. <br>
							마이페이지로 이동하셔서 기관정보를 등록해 주세요. <br>
							마이페이지로 이동하시겠습니까?
			</p>
		    <div class="popup_button_area_center">
				<button type="button" class="blue_btn mr5" onclick="location.href='/member/fwd/mypage/institution'">이동</button>
				<button type="button" class="gray_btn popup_close_btn" onclick="$('.institution_warning_box, .popup_bg').fadeOut(350);">취소</button>
			</div>	
		</div>									
	</div> 
</div>
