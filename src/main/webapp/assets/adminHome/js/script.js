...
/*------------------------------------------------------sub------------------------------------------------*/
	...
	*/
		...
		$('.list_table table #allcheck').on('click', function(e) {
			const checkall = $('.list_table table #allcheck');
	...
	//시간대별 통계 
		...
		$('.statisticsdate2 .sum_result').text(timeSumResultsum);
			...
			//써치필드 버튼 클릭 컬러 체인지
			$('.day_class_btn_area button').on('click', function(e) {
				$(this).addClass('active');	
				$(this).siblings().removeClass('active');
				$('.statistics_list_searchbox .datepicker_area #join_datepicker1').val('시작일');
				$('.statistics_list_searchbox .datepicker_area #join_datepicker2').val('종료일');
			});
			
			$('.statistics_list_searchbox .datepicker_area button').on('click', function(e) {
				$('.statistics_list_searchbox').find('button.active').removeClass('active');
			});
	...
	//-----poster page
		...
		//추가
			$('.poster_list_add').on('click', function(e) {
				...
				var addStaffText = '<li class="clearfix li_active">' + 
										'<div class="business_poster_area clearfix">' +
											'<label for="business_posters_title" class="hidden">제목</label>' +
											'<input id="ibx_title" title="제목" type="text" class="form-control business_poster_title" id="business_posters_title" />' +
											'<div class="poster_image-container image-container">' +
												'<a href="" target="_blank">' +
													'<img class="poster_preview-image" src="https://dummyimage.com/300x400/ffffff/000000.png&text=preview+image" alt="포스터 이미지" />' +
												'</a>' +
												...
												'<input class="poster_upload-name upload-name" id="upload-name" value="첨부파일" placeholder="첨부파일">' +
											'</div>' +
											'<div class="business_poster_text_area add_form fr">' +
												'<textarea id="txa_description" title="내용" name="business_poster_text_title_outline_c1" class="business_poster_text"></textarea>' +
											'</div>' +
										'</div>' +
										'<div class="buttonarea fr">' +
											'<button type="button" class="blue_btn modify_btn mr5">수정</button>' +
											'<button type="button" class="gray_btn poster_list_del">삭제</button>' +
											'<button type="button" class="blue_btn poster_list_add_complate mr5">등록완료</button>' +
											'<button type="button" class="gray_btn2 add_poster_cancel">취소</button>' +
										'</div>' +
									'</li>';
		...
		$(document).on("click",".poster_image-container label, .image-container label",function(){ 
			$(this).parent().addClass('active');
			$(this).parent().parent().parent().siblings('li').children().children('.poster_image-container, .image-container label').removeClass('active');
		});
        function handleImgFileSelect(e) {
            var files = e.target.files;
            var filesArr = Array.prototype.slice.call(files);
            ...
            filesArr.forEach(function(f) {
                ...
                reader.onload = function(e) {					
                    ...
                    $(".active .poster_upload-name, .poster_upload-name_sub").val(fileName);
                }
                reader.readAsDataURL(f);
            });
        }
