...
$(function(){
		...
		$(window).resize(function(){ 			
			if (window.innerWidth < 1023) {
				$(window).on('scroll',function(){ 
					const scrollM = $(window).scrollTop();
					if (scrollM > 1) {
				...
				$(window).on('scroll',function(){ 
					const scrollPc = $(window).scrollTop();
					if (scrollPc > 1) {
	...
	$('.search_txt_del').on('click', function(e) {
		...
		//circle 안에 span 클래스 추가

	$(document).ready(function() {
		$('.summary_processor ul li .circle span:first-child').addClass('spanadd');
		$('.summary_maching_purpose ul li, .summary_purpose p, .summary_maching_purpose_box li, .summary_maching_purpose li, .summary_maching_purpose p, .summary_purpose p, .summary_requ_area p, .summary_announcement_purpose_box ul, .summary_announcement_system_box ul').addClass('w_p')
	});	
